...
class TransferBetweenSaifuAndDebtViewSetTests(APITestCase):
    ...
    def test_transfer_from_saifu_to_debt_Correctly(self):
        ...
        post_response = self.client.post('/transfer_between_saifu_and_debt/', input_data, format='json',
                                         HTTP_AUTHORIZATION='JWT {}'.format(self.jwt_token))
        self.assertEqual(post_response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(set(post_response.data.keys()), {'id', 'transfer_date', 'amount', 'note', 'u_saifu', 'u_debt'})
        updated_u_saifu = USaifu.objects.get(pk=self.u_saifu.id)
        self.assertEqual(updated_u_saifu.current_balance, 0)
        updated_u_debt = UDebt.objects.get(pk=self.u_debt.id)
        self.assertEqual(updated_u_debt.current_principal_amount, 10000)
        self.assertEqual(updated_u_debt.current_gained_amount, 10000)
    ...
    def test_transfer_from_debt_to_saifu_Correctly(self):
        ...
        self.assertEqual(updated_u_saifu.current_balance, 20000)
        updated_u_debt = UDebt.objects.get(pk=self.u_debt.id)
        self.assertEqual(updated_u_debt.current_principal_amount, 20000)
        self.assertEqual(updated_u_debt.current_gained_amount, 30000)
