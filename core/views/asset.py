...
from core.models.transaction.asset import TAssetEvaluate
from ..serializers.asset import AssetCategorySerializer, AssetSerializer, AssetEvaluateSerializer
...
class AssetEvaluateViewSet(viewbase.IsOwnerOnlyViewSetBase):
    """
    A ViewSet for Asset Evaluate Transaction
    """
    serializer_class = AssetEvaluateSerializer

    def get_queryset(self):
        return TAssetEvaluate.objects.filter(owner=self.request.user)
    ...
    @transaction.atomic
    def create(self, request, *args, **kwargs):
        serializer = AssetEvaluateSerializer(data=request.data)
        if serializer.is_valid():
